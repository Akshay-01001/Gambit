...
export function findMatch(
    playerId: string,
    message: { payload: { game_type: string; game_time: number } },
) {
    ...
    for (const [waitingId, prefs] of gameManager
        .getWaitingPlayers()
        .entries()) {
        if (
            waitingId !== playerId &&
            prefs.game_type === message.payload.game_type &&
            prefs.game_time === message.payload.game_time
        ) {
    ...
    if (opponentId) {
        ...
        createGame(
            playerId,
            opponentId,
            message.payload.game_type,
            message.payload.game_time,
        );
    } else {
        gameManager.addToWaiting(playerId, {
            game_type: message.payload.game_type,
            game_time: message.payload.game_time,
        });
        ...
        setTimeout(() => {
            if (gameManager.getWaitingPlayers().has(playerId)) {
                ...
                if (currentPlayer) {
                    gameManager.safeSend(currentPlayer.ws, {
                        type: SocketEvents.NO_MATCH_FOUND,
                    });
                }
            }
        }, 30000);
...
export async function createGame(
    ...
    if (
        player1.ws.readyState !== player1.ws.OPEN ||
    ...
    try {
        ...
        if (!game) {
            gameManager.safeSend(player1.ws, {
                type: SocketEvents.ERROR,
                message: 'Failed to create game',
            });
            gameManager.safeSend(player2.ws, {
                type: SocketEvents.ERROR,
                message: 'Failed to create game',
            });
            return;
...
export async function handleRejoin(
    userId: string,
    gameId: string,
...
export async function makeMove(
    ...
    try {
        ...
        if (gameOver) {
            ...
            const nextTimeLeft =
                game.turn === 'w' ? game.whiteTimeLeft : game.blackTimeLeft;
        ...
        gameManager.safeSend(ws, {
            type: SocketEvents.ERROR,
            message: 'Server error while processing move',
        });
    }
}
...
export async function handleDrawAccept(
    gameId: string,
    userId: string,
    ws: AuthenticatedWebSocket,
) {
