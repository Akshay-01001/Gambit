...
interface DrawModalProps {
    handleDrawModalOpen: (open: boolean) => void;
}

const DrawModal: React.FC<DrawModalProps> = ({ handleDrawModalOpen }) => {
    const handleDrawGame = () => {
        gameManager.offerDraw();
        handleDrawModalOpen(false);
    };
            ...
            <div className="bg-card w-full max-w-sm rounded-2xl border border-[#2c2c2a] shadow-2xl p-6 flex flex-col gap-4">
                ...
                <div className="flex gap-3 mt-4">
                    <button
                        ...
                        onClick={() => handleDrawModalOpen(false)}
                    >
                        Cancel
                    </button>
                    <button
                        className="flex-1 py-3 bg-destructive hover:bg-destructive/90 transition-colors rounded-xl text-white font-bold shadow-lg cursor-pointer"
                        onClick={handleDrawGame}
                    >
                        Draw
                    </button>
                </div>
