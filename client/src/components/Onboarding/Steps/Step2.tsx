...
const Step2 = () => {
    ...
    const handleSelectDefaultAvatars = useCallback(
        (url: string) => {
            ...
            <form onSubmit={handleSubmit}>
                <div className="mt-10 space-y-8">
                    <div className="flex items-center gap-4 rounded-xl border border-border bg-card p-5">
                        <img
                            ...
                            alt="Piece"
                            className="h-16 w-16 transition rounded-full object-cover"
                        />
                            ...
                            {avatar_urls.map((url, index) => {
                                ...
                                return (
                                    <img
                                        ...
                                        key={index}
                                        alt="Piece"
                                        className={`h-16 w-16 transition rounded-full cursor-pointer ${isSelected && 'ring-3 ring-primary'}`}
                                        onClick={() =>
                                            handleSelectDefaultAvatars(url)
                                        }
                                    />
                                );
                            })}
                            <label htmlFor="avatar">
                                <div className="grid h-16 w-16 place-items-center rounded-full border-2 border-dashed border-border text-muted-foreground transition hover:border-primary hover:text-primary cursor-pointer">
                                    <svg
                ...
                <div className="mt-8">
                    <button
                        type="submit"
                        disabled={isLoading}
                        className="w-full cursor-pointer bg-primary text-primary-foreground h-10 px-4 py-2 rounded-md font-medium transition-colors hover:bg-primary/90  disabled:opacity-50"
                    >
                        Continue
                    </button>
                </div>
